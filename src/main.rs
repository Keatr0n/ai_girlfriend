...
use crate::{shutdown::save_conversation, state::{LlmCommand, LlmState, StateHandle}};
...
fn main() -> anyhow::Result<()> {
    ...
    let llm_model_path = selected.llm_model_path.clone()
        .unwrap_or_else(|| config.global.default_llm_model_path
    ...
    let piper_model_path = selected.piper_model_path.clone()
        .unwrap_or_else(|| config.global.default_piper_model_path
    ...
    vad::run_vad(state_for_vad, audio, source_rate, |utterance| {
        if let Ok(text) = stt.transcribe(&utterance) {
            ...
            if current_state.is_only_responding_after_name && match current_state.time_since_name_was_said {None => true, Some(instant) => instant.elapsed().as_secs() > 5} {
                let words = text.split(" ");

                if words.clone().count() <= 3 && words.clone().any(|word| selected.name.eq_ignore_ascii_case(&word.chars().filter(|c| c.is_alphabetic()).collect::<String>())) {
                    state.update(|s| {
                        s.time_since_name_was_said = Some(Instant::now());
                        s.tts_command = Some("Yes?".into());
                    });

                    return;
                }

                if !words.take(5).any(|word| selected.name.eq_ignore_ascii_case(&word.chars().filter(|c| c.is_alphabetic()).collect::<String>())) {
                    return
                }
            }
